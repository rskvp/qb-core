...
func (w *FileWatcher) SetFilterCreateMoveRemove() {
	w.FilterOps(Move, Create, Remove)
}
...
func (w *FileWatcher) Add(name string) (err error) {
	...
	isHidden, err := qb_utils.Paths.IsHiddenFile(name)
	if err != nil {
		return err
	}
...
func (w *FileWatcher) Start(d time.Duration) error {
	...
	// Make sure the FileWatcher is not already running.
	w.mu.Lock()
	if w.running {
		w.mu.Unlock()
...
func (w *FileWatcher) retrieveFileList() map[string]os.FileInfo {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *FileWatcher) pollEvents(files map[string]os.FileInfo, evt chan Event,
	cancel chan struct{}) {
	w.mu.Lock()
	defer w.mu.Unlock()
	...
	for path1, info1 := range removes {
		for path2, info2 := range creates {
			if qb_utils.Paths.IsSameFileInfo(info1, info2) {
				e := Event{
					Op:       Move,
					Path:     path2,
					OldPath:  path1,
...
func (w *FileWatcher) list(name string) (map[string]os.FileInfo, error) {
	fileList := make(map[string]os.FileInfo)
...
outer:
	for _, fInfo := range fInfoList {
		...
		isHidden, err := qb_utils.Paths.IsHiddenFile(path)
		if err != nil {
			return nil, err
		}
...
func (w *FileWatcher) listRecursive(name string) (map[string]os.FileInfo, error) {
	fileList := make(map[string]os.FileInfo)
