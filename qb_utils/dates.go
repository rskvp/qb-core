...
func (instance *DatesHelper) ToDateTime(value interface{}) (t time.Time) {
	if dt, b := value.(time.Time); b {
		t = dt
	} else if s, b := value.(string); b {
		dt, err := instance.TryParseAny(s)
		if nil == err {
			t = dt
		}
	} else if i, b := value.(int64); b {
		dt = time.Unix(i, 0)
		t = dt
	}
	return
...
func (instance *DatesHelper) FirstAndLastOfMonth(date interface{}, keepTime bool) (firstOfMonth time.Time, lastOfMonth time.Time) {
	dt := instance.ToDateTime(date)
	currentYear, currentMonth, _ := dt.Date()
	currentLocation := dt.Location()

	if keepTime {
		firstOfMonth = time.Date(currentYear, currentMonth, 1, dt.Hour(), dt.Minute(), dt.Second(), dt.Nanosecond(), currentLocation)
	} else {
...
func (instance *DatesHelper) LastDayOfMonth(date interface{}, day time.Weekday, includeDate, keepTime bool) time.Time {
	...
	if includeDate && dt.Weekday() == day {
		...
		_, dt = instance.FirstAndLastOfMonth(dt, keepTime)
		for {
			if dt.Weekday() == day {
				return dt
			}
...
func (instance *DatesHelper) FirstDayOfMonth(date interface{}, day time.Weekday, includeDate, keepTime bool) time.Time {
	...
	if includeDate && dt.Weekday() == day {
		...
		dt, _ = instance.FirstAndLastOfMonth(dt, keepTime)
		for {
			if dt.Weekday() == day {
				return dt
			}
...
func (instance *DatesHelper) FormatDate(dt time.Time, pattern string) string {
	return dt.Format(toGoDateLayout(pattern))
}

func (instance *DatesHelper) FormatDateLocale(dt time.Time, lang string) string {
	pattern := ""
	country := Strings.Split(lang, "_-")[0]
	if v, b := LocalePatterns[country]; b {
		pattern = v[0]
	} else {
		pattern = LocalePatterns["*"][0]
	}
	return dt.Format(toGoDateLayout(pattern))
}
...
func (instance *DatesHelper) ParseDate(dt string, pattern string) (time.Time, error) {
	return time.Parse(toGoDateLayout(pattern), dt)
}

func (instance *DatesHelper) TryParseAny(any interface{}) (time.Time, error) {
	if d, ok := any.(time.Time); ok {
		return d, nil
	} else {
		dateStr := fmt.Sprintf("%v", any)
		// try a standard layout
		if t, err := time.Parse("2006-01-02T15:04:05.000Z", dateStr); nil == err {
			return t, nil
		}

		// try all default patterns
		for _, pattern := range DatePatterns {
			t, err := instance.ParseDate(dateStr, pattern)
			if nil == err {
				return t, nil
			}
		}
		...
		// try advanced parser
		if t, err := instance.ParseAny(dateStr); nil == err {
			return t, nil
		}
	}
	return time.Time{}, errors.New("unable_to_parse")
}
...
// dateStr, it uses the given location rules for any zone interpretation.
...
func toGoDateLayout(pattern string) string {
