...
func TestSplit_invalid(t *testing.T) {
	...
	if _, err := Shamir.Split(secret, 0, 0); err == nil {
	...
	if _, err := Shamir.Split(secret, 1000, 3); err == nil {
		t.Fatalf("expect error")
	}

	if _, err := Shamir.Split(secret, 10, 1); err == nil {
		t.Fatalf("expect error")
	}
}
...
func TestCombine_invalid(t *testing.T) {
	...
	if _, err := Shamir.Combine(parts); err == nil {
		t.Fatalf("should err")
	}
